...
interface WidthConstraintsProps {
  minWidth: string
  maxWidth: string
  width: string
}
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
  (
    ...
    const getResponsiveSettings = (): ResponsiveSettingsProps => {
      if (!responsive) {
        return {
          maxCount: maxCount,
          hideIcons: false,
    ...
    const getBadgeAnimationClass = (): string => {
    ...
    const getPopoverAnimationClass = (): string => {
    ...
    const toggleOption = (optionValue: string): void => {
      if (disabled) return
      const check = selectedValues.includes(optionValue)
      if (selectedValues.length >= 4 && !check) return
      const option = getOptionByValue(optionValue)
      if (option?.disabled) return
      const newSelectedValues = check
        ? selectedValues.filter((value) => value !== optionValue)
        : [...selectedValues, optionValue]
      if (newSelectedValues.length == 4) {
        setSelectedValues(newSelectedValues)
        onValueChange(newSelectedValues)
        setIsPopoverOpen(false)
      } else {
    ...
    const getWidthConstraints = (): WidthConstraintsProps => {
        ...
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen} modal={modalPopover}>
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              className={cn(
                'flex p-1 rounded-md border min-h-12 h-auto items-center justify-between bg-inherit hover:bg-inherit [&_svg]:pointer-events-auto',
                      ...
                      .map((value) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className={cn(screenSize === 'mobile' && 'truncate', 'ml-0.5')}>
                              {option.label}
                            </span>
                            <div
                              role="button"
                              ...
                              className="ml-2 h-5 w-5 cursor-pointer hover:bg-white/20 rounded-sm p-0.5 -m-0.5 focus:outline-none focus:ring-1 focus:ring-white/50"
                            >
                              <XCircle
                                className={cn(
                                  'h-3 w-3',
                    ...
                    {filteredOptions.map((option) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={cn(
                              'mr-2 flex size-4 items-center justify-center border border-input rounded-[4px]',
                              isSelected
                                ? 'bg-primary text-primary-foreground'
